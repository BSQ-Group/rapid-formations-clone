...
## `bg-[var(--token)]` silently drops a gradient token — Tailwind compiles it to `background-color`

- **When it bites:** A theme token holds a `linear-gradient(...)` (`--surface-promo-*`, `--surface-ad-banner`, `--surface-team-card-*`). Writing `bg-[var(--surface-team-card-body)]` produces `background-color: var(...)`, and a gradient is not a valid colour, so the element paints **transparent**. Nothing errors; the block just looks flat. The same ambiguity bites in reverse when a colour token sits beside a `bg-[url(...)]` — Tailwind then guesses `background-image` and the colour is dropped.
- **Detect:**
  ```bash
  # Any bare var() behind bg-[ ] is ambiguous. Both hints must be explicit.
  grep -rn "bg-\[var(--" src/blocks src/components | grep -v "bg-\[image:var\|bg-\[color:var"
  # And confirm it actually paints, in the browser, not by reading the class:
  #   getComputedStyle(el).backgroundImage  -> must not be "none"
  #   getComputedStyle(el).backgroundColor  -> must not be "rgba(0, 0, 0, 0)"
  ```
- **Fix:** Always tell Tailwind which property it is: `bg-[image:var(--gradient-token)]` for a gradient, `bg-[color:var(--colour-token)]` for a colour — the latter is mandatory whenever the same class list also sets a `bg-[url(...)]`.

## A Radix dialog is portaled to `<body>`, so it loses the block's font and theme classes

- **When it bites:** A block sets `font-legacy-condensed` on its section wrapper and everything inside inherits it — except a `DialogContent`, which React portals to `document.body`, outside that wrapper. The panel silently falls back to the app default (Inter). It is easy to miss because the trigger and the cards behind the dialog are correct, and the fallback is a perfectly readable sans. Measured symptom: the same bold label was 145.2px wide in the dialog and 128.7px on the source.
- **Detect:**
  ```bash
  # Open the dialog, then compare the resolved family inside it against the block:
  #   getComputedStyle(document.querySelector('[role="dialog"] b')).fontFamily.split(',')[0]
  #   getComputedStyle(document.querySelector('main .font-legacy-condensed')).fontFamily.split(',')[0]
  # Anything that differs is the portal, not the CSS.
  grep -rln "DialogContent" src/blocks | xargs grep -Ln "font-legacy-condensed"
  ```
- **Fix:** Repeat the font (and any theme class the panel depends on) on the `DialogContent`'s own className. Same applies to tooltips, popovers, sheets and toasts — anything Radix portals.

## A `flex flex-col` `DialogTrigger` shrinks its children — the UA stylesheet forces `align-items: center` on `<button>`

- **When it bites:** A whole card is made clickable by putting the card's layout classes on `DialogTrigger`, which renders a `<button>`. Chrome's UA stylesheet sets `align-items: center` on buttons, which overrides flex's `stretch` default, so every child sizes to its **content** instead of the card width. Measured symptom: a 350px card whose photo panel came out 250px and whose body came out 298px, centred, with the background gradients visibly inset.
- **Detect:**
  ```bash
  # Any flex/grid container that is also a button needs an explicit cross-axis rule:
  grep -rn "DialogTrigger className" src/blocks -A2 | grep -E "flex-col|grid" | grep -v "items-stretch"
  # In the browser, the tell is a child narrower than its parent with no width class of its own:
  #   el.getBoundingClientRect().width !== el.parentElement.getBoundingClientRect().width
  ```
- **Fix:** Add `items-stretch` (or `w-full` on each child). The same applies to `<button>` used as a grid container, and to `Button asChild` wrapping a layout.
