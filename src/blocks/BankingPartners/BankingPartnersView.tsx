...
import type { BankingPartnersBlock, BusinessBankAccount } from '@/payload-types'
...
// Presentational grid. Content comes from the `businessBankAccounts` global
// (see Component.tsx); this component is pure so Storybook can render it with
// mock data — the same split as BusinessBankAccounts + BusinessBankAccountsCarousel.
export type BankingPartnersViewProps = Pick<
  BusinessBankAccount,
  'heading' | 'subheading' | 'backgroundPattern' | 'banks' | 'cta'
> & { sectionLayout?: BankingPartnersBlock['sectionLayout'] }

export const BankingPartnersView: React.FC<BankingPartnersViewProps> = ({
  heading,
  subheading,
  backgroundPattern,
  banks,
      ...
      <Container>
        ...
        <ul className={s.grid}>
          {banks?.map((bank, index) => {
            const style: React.CSSProperties = { backgroundColor: bank.brandColour ?? undefined }
            if (patternUrl) {
