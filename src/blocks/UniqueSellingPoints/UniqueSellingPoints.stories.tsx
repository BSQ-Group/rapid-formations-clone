...
const EXTREME_DESCRIPTION =
  'Over 750,000 limited companies formed since 2010, with thousands of excellent independent reviews on Google and Trustpilot from founders who came back to us for their second and third company, and a support team that answers the telephone in under a minute seven days a week, including bank holidays, because a company that cannot be reached is not much of a company formation agent.'

const extremePoints: UniqueSellingPointsBlockProps['points'] = points!.map((p) => ({
  ...p,
  description: EXTREME_DESCRIPTION,
}))

export const ExtremeCopy: Story = {
  args: { ...defaultArgs, points: extremePoints },
}
...
export const NoIcons: Story = {
  args: { ...defaultArgs, points: points!.map((p) => ({ ...p, icon: null })) } as any,
}

export const SomeIconsMissing: Story = {
  args: {
    ...defaultArgs,
    points: points!.map((p, i) => (i % 2 ? { ...p, icon: null } : p)),
  } as any,
}
