export const videoLibraryStyles = {
  ...
  navMobile: 'relative block md:hidden',
  navDesktop: 'hidden md:block',

  dropdown:
    'w-full cursor-pointer appearance-none rounded-lg border-2 border-solid border-[color:var(--border-on-light-select)] bg-[var(--surface-canvas)] py-3 pr-10 pl-4 text-lg leading-[14px] text-[var(--text-on-light-muted)] focus:border-[color:var(--border-on-light-select)] focus:outline-none',

  dropdownChevron:
    'pointer-events-none absolute top-1/2 right-2 h-[27px] w-[27px] -translate-y-1/2 text-[var(--text-on-light-muted)]',
  ...
  categoryTitle:
    'mb-8 block text-left text-[30px] leading-[1.235] font-bold text-[var(--text-on-light-base)] md:text-[36px]',
  ...
  thumbnail: 'relative mb-4 block w-full overflow-hidden',
  ...
  meta: 'mt-2 flex items-center justify-between text-base text-[var(--text-on-light-muted)]',
